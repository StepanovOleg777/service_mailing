...
class Client(models.Model):
    ...
    email = models.EmailField(
        _("email"), unique=True, help_text=_("Email address of the client.")
    )
    full_name = models.CharField(
        _("full name"), max_length=255, help_text=_("Full name of the client.")
    )
...
class Message(models.Model):
    ...
    subject = models.CharField(
        _("subject"), max_length=255, help_text=_("Email subject line.")
    )
    body = models.TextField(_("body"), help_text=_("Email body content."))
    owner = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        verbose_name=_("owner"),
...
class Mailing(models.Model):
    ...
    start_time = models.DateTimeField(
        _("start time"), help_text=_("Date and time when mailing can start.")
    )
    end_time = models.DateTimeField(
        _("end time"), help_text=_("Date and time when mailing must end.")
    )
...
class MailingAttempt(models.Model):
    ...
    server_response = models.TextField(
        _("server response"), blank=True, help_text=_("Response from the mail server.")
    )
